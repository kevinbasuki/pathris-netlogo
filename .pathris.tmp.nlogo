...
to setup
  clear-all
  ;;Create turtles with shape l, size 3, random position, and facing 0.
  create-turtles 4
  [
    set shape "l"
    set size 3
...
to make-goal
  if mouse-down? [
    ...
    set goal patch mouse-xcor mouse-ycor
    display
  ]
end
...
to-report total-expected-cost [#goal]
  ; f(n) = g(n) - cost sekarang + h(n) - cost ke goal
  report cost-path + heuristic #goal
end

to-report A* [#start #goal #valid-map #final-cost]
  ; Bersihin semua valid-map (bukan tembok) yang udah dikunjungin
  ...
  ; Nge-set #Start sebagai awal, set visited dan active dengan true
  ask #Start
  [
    set father self
    set visited? true
  ...
  ; Set variable exists (ada jalur) sebagai true, entar mungkin bisa false
  let exists? true
  ; Selama belom nyampe goal dan masih mungkin ada, loop
  while [not [visited?] of #goal and exists?]
  [
    ; Set options sebagai semua valid-map yang aktif
    let options #valid-map with [active?]
    ; Cek apakah ada valid-map, kalo gaada exists di-set false
    ifelse any? options
    [
      ; Ambil option dengan f(n) paling rendah
      ask min-one-of options [total-expected-cost #goal]
      [
        ; Set cost-path-father dengan cost-path yang lagi dicek
        let cost-path-father cost-path
        ; Set grid sekarang sebagai false
        set active? false
        ; Cek neighbors yang valid (bukan tembok)
        let valid-neighbors neighbors4 with [member? self #valid-map]
        ask valid-neighbors
        [
          ; Set nilai t (temporary) dengan angka yang sangat tinggi jika sudah di-visit
          ; Jika belom, set dengan nilai f(n)
          let t ifelse-value visited? [ total-expected-cost #goal] [2 ^ 20]
          ; Kalo temporary lebih besar, set yang sedang di-cek dengan informasi parent pemanggil
          ; Disini ada pengecekan yang lebih efektif, yang paling kecil akan di-set di-patch
          if t > (cost-path-father + distance myself + heuristic #Goal)
          [
            ; Set patch dengan parent pemanggil
            set father myself
            set visited? true
            set active? true
            ; Simpan nilai cost-path pada patch tersebut, cost-patch paling minimum
            set cost-path cost-path-father + distance father
            set #final-cost precision cost-path 3
    ] ] ] ]
    ; Kalo gada lagi pilihan, di-set false
    [
      set exists? false
    ] ]
  ; Sehabis pengecekan, cek exists, kalo true
  ; Masukin semua ke dalam list, dimulai dari goal (child) ke start (parent)
  ifelse exists?
  [
    let current #goal
